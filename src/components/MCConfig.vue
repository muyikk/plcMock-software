<template>
        ...
        <el-tab-pane label="基础配置">
          <div class="baseConfig">
            ...
            <div class="input">
              ...
              <el-input type="text" placeholder="Qna-3E协议" disabled style="width: 200px" />
            </div>
          </div>
  ...
  <el-drawer v-model="drawer" direction="btt" size="80%" :with-header="false">
    <viewListMC :params="pollingParams" @update-value="update" />
  </el-drawer>
...
export default {
  ...
  methods: {
    ...
    async load() {
      ...
      window.ipcRenderer.loadMC(filePaths);
    ...
    update({ param, newValue }) {
      window.ipcRenderer.updateMC({ param, newValue })
    },
    polling() {
      window.ipcRenderer.pollingMC()
    },
  },
  mounted() {
    // window.ipcRenderer.onInitMCResponse((response) => {
    //   if (response.success) {
    //     console.log('MC param init successfully!');
    //     ElNotification({
    //       title: '成功',
    //       message: 'MC服务初始化完成',
    //       type: 'success'
    ...
    //   }
    // });
    window.ipcRenderer.onPollingMCResponse((response) => {
      if (response.success) {
    ...
    window.ipcRenderer.onStartMCResponse((response) => {
      if (response.success) {
    ...
    window.ipcRenderer.onUpdateMCResponse((response) => {
      // if (response.success) {
    ...
    window.ipcRenderer.onCloseMCResponse((response) => {
      if (response.success) {
        console.log('server close successfully!');
        ElNotification({
          title: '警告',
    ...
    window.ipcRenderer.onSaveMCResponse((response) => {
      this.loading.save = false
      if (response.success) {
    ...
    window.ipcRenderer.onLoadMCResponse((response) => {
      let data = JSON.parse(response?.data)
      if (response.success && data.name != 'MC') {
        ElNotification({
          title: '错误',
          message: '该配置不是MC配置',
          type: 'error'
        });
        // throw Error('该配置不是MC配置')
